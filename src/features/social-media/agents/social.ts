...
import { baseSystemPrompt } from "@/lib/agents/base-system-prompt";
import { runTextLLM } from "@/lib/agents/llm";
import { addMessages, getMessages } from "@/lib/agents/memory";
import type { MessageType } from "@/lib/agents/types";

import { insertComment, updateComment } from "@/features/social-media/api/mutations";
import { getCachedIntegrationByEntityIdAndStrategy } from "@/features/social-media/api/queries";
...
interface SocialCommentAgentProps extends SocialAgentBaseProps {
  type: "comment";
  commentId: string;
  entityId: string; // Optional for comments
...
export async function runSocialAgent(props: SocialAgentProps) {
  ...
  if (props.type === "comment") {
    await insertComment({
      ...
      commenterId: props.senderId,
      commenterUsername: props.senderUsername,
      commentText: props.message,
  ...
  if (props.type === "comment") {
    await updateComment(integration.agentId, props.commentId, {
      aiResponse: response,
