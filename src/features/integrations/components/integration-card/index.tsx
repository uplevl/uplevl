...
import { deleteIntegration } from "@/features/integrations/actions/services";
import { useIntegrationId, useIsIntegrated } from "@/features/integrations/providers/integrations-provider";
...
export function IntegrationCard({ title, description, icon, strategy }: IntegrationCardProps) {
  const integrated = useIsIntegrated(strategy);
  const integrationId = useIntegrationId(strategy);
      ...
      <CardContent>
        <div className="flex items-center justify-center gap-x-5">
          ...
          <div className="flex flex-col gap-y-2">
            <Button onClick={handleConnect} disabled={integrated} className="min-w-28">
