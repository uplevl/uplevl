...
import { useAgent } from "@/features/agent/providers/agent-provider";
import { insertPost } from "@/features/posts/actions/posts";
...
export function CreatePostsFormProvider({ children }: CreatePostsFormProviderProps) {
  const { user } = useUser();
  ...
  async function handleSubmit() {
    ...
    await Promise.all(
      uploadedFiles.map(async (image) => {
        const post = await runPostAgent({ userId: user.id, imageUrl: image, description: description });
